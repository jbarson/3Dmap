...
const surfaceUniforms = {
  ...
  uSpecularMap: { value: null as THREE.Texture | null },
...
function switchPlanet(name: string, updateUrl = true) {
  ...
  const starmapLink = document.getElementById("starmap-link") as HTMLAnchorElement | null;
  ...
  if (p.starTemp) {
    sunLight.color.copy(kelvinToRGB(p.starTemp));
  } else {
  ...
  const loadTexture = (suffix: string): Promise<THREE.Texture> => {
